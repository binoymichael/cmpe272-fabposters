...
class Movie {
	public static function all($database_connection) {
		try {
			$sql = 'SELECT id, name, quantity FROM movies where 1 ';
			$sql .= ' ORDER BY id';
	...
	public static function find($database_connection, $movie_id) {
		try {
			$sql = 'SELECT id, name, quantity FROM movies where id = :id LIMIT 1';
			$query = $database_connection->prepare($sql);
	...
	public static function find_set($database_connection, $movie_ids) {
		try {
			$sql = 'SELECT id, name, quantity FROM movies where FIND_IN_SET(id, :ids)';
			$query = $database_connection->prepare($sql);
